'''
Code base https://github.com/shikiw/OPERA/blob/main/chair_eval.py
'''
import os
import sys
import nltk
...
class CHAIR(object):
    ...
    def _load_generated_captions_into_evaluator(self, cap_file, image_id_key, caption_key, cap_file2=None):
        ...
        if cap_file2 is not None:
            self.caps2, self.eval_imids2 = load_generated_captions(cap_file2, image_id_key, caption_key)
    ...
    def compute_chair(self, cap_file, image_id_key, caption_key, cap_file2=None):
        '''
        Given ground truth objects and generated captions, determine which sentences have hallucinated words.
        '''
        self._load_generated_captions_into_evaluator(cap_file, image_id_key, caption_key, cap_file2)
        ...
        for i in tqdm.trange(len(caps)):
            ...
            if cap_file2 is not None: 
                words2, node_words2, idxs2, raw_words2 = self.caption_to_words(cap2)
                len_caps, len_caps2 = len(raw_words), len(raw_words2)
                if len_caps2 < len_caps:
                    raw_words = raw_words[:len_caps2]
                    cap = ' '.join(raw_words).replace(' ,', ',').replace(' .', '.')
                    words, node_words, idxs, raw_words = self.caption_to_words(cap) 
...
if __name__ == '__main__':
    ...
    parser.add_argument("--cap_file", type=str, default='./log/llava-1.5-13b-hf/greedy_base_AvoidNewLine',
                        help="path towards json or jsonl saving image ids and their captions in list of dict.")
    parser.add_argument("--cap_file2", type=str, default=None)
    ...
    parser.add_argument("--coco_path", type=str, default='/Users/zongbo/Data/COCO_2014/annotations_trainval2014/annotations',
    ...
    capfile = glob.glob(os.path.join(args.cap_file, 'prompt*.jsonl'))[0]
    if args.cap_file2 is not None:
        capfile2 = glob.glob(os.path.join(args.cap_file2, 'prompt*.jsonl'))[0]
        cap_dict = evaluator.compute_chair(capfile, args.image_id_key, args.caption_key, capfile2)
    else:
        cap_dict = evaluator.compute_chair(capfile, args.image_id_key, args.caption_key) 

    save_path = args.cap_file
    print_metrics(cap_dict)
    if args.cap_file2 is not None:
        save_name = args.cap_file2.split('/')[-1]
    else:
        save_name = ''
    with open(os.path.join(save_path, save_name+'_metrics.json'), 'w') as file:
        json_str = json.dumps(cap_dict['overall_metrics'])
        file.write(json_str)
    
    # save_hallucinated_words(os.path.join(save_path, "evaluated.json"), cap_dict)
